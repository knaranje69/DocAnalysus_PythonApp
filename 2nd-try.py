    ...
    def process_file(self, *args):
        try:
            if self.file_path:
                ...
                #Create a popup window to display the response
                popup_content = BoxLayout(orientation='horizontal',  )
                scroll_view = ScrollView()
                answer_label = Label(text=answer, halign='left', valign='middle', size_hint_y=None)
                ...
                popup = Popup(title='Response', content=popup_content, size_hint=(None, None), size=(1000, 800))
                popup.open()
