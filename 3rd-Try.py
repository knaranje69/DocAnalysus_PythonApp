...
class MainLayout(BoxLayout):
    ...
    def create_popup(self, answer):
        popup_content = BoxLayout(orientation='vertical')
        response_container = BoxLayout(orientation='vertical', padding=20)
        scroll_view = ScrollView(do_scroll_x=False, do_scroll_y=True)
        response_label = Label(text=answer, halign='left', valign='top', size_hint_y=None, text_size=(Window.width - 40, None))
        response_label.bind(texture_size=response_label.setter('size'))
        scroll_view.add_widget(response_label)
        response_container.add_widget(scroll_view)
        popup_content.add_widget(response_container)
